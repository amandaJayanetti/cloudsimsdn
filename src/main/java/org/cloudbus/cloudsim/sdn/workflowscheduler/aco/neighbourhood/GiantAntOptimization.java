...
import org.cloudbus.cloudsim.sdn.workflowscheduler.aco.neighbourhood.GiantAnt;
...
public class GiantAntOptimization {
    ...
    private List<GiantAnt> ants = new ArrayList<>();
    ...
    public GiantAntOptimization(List<SDNHost> hostList, List<SDNVm> vmList, VmAllocationPolicyToTasks policy, List<SDNHost> predecessors) {
        ...
        IntStream.range(0, numberOfAnts)
                .forEach(i -> {
                    ...
                    ants.add(new GiantAnt(numberOfHosts, taskVms, hostList));
                });
    }

    public GiantAntOptimization(List<SDNHost> hostList, List<SDNVm> vmList, VmAllocationPolicyToTasks policy) {
        this.hostList = hostList;
        graph = policy.getHostMatrix();
        numberOfHosts = graph.length;
        numberOfAnts = (int) (numberOfHosts * antFactor);
    ...
    public GiantAntOptimization(List<SDNHost> hostList, List<SDNHost> eligibleHosts, List<SDNVm> vmList, List<SDNHost> currentClique, VmAllocationPolicyToTasks policy) {
        this.hostList = hostList;
        ...
        for (int i = 0; i < ants.size(); i++) {
            ...
            ants.add(new GiantAnt(numberOfHosts, taskVms, hostList, eligibleHosts, currSelectedHosts));
        }
     ...
     * Select next host
     */
    private SDNHost selectNextCity(GiantAnt ant) {
     ...
     */
    public void calculateProbabilities(GiantAnt ant) {
        ...
        for (SDNHost candidateHost : candidateHosts) {
            ...
            for (SDNHost currHost : currClique) {
                ...
                phermoneFactor += Math.pow(1/(trails[hostList.indexOf(candidateHost)][hostList.indexOf(currHost)] + 0.000001), 10); // to address phermone zero case
            }
    ...
    private void updateTrails() {
        ...
        GiantAnt bestAntInCurrIteration = ants.get(0);
