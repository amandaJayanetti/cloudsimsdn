 ...
 */
public class SDNCustomBroker extends SDNBroker {
	...
	public SDNCustomBroker(String name) throws Exception {
		super(name);
	...
	public void printResult() {
		...
		// For group analysis		
		int[] groupNumWorkloads = new int[SDNCustomBroker.lastAppId];
		double[] groupTotalServetime = new double[SDNCustomBroker.lastAppId];
		double[] groupTotalServetimeCPU = new double[SDNCustomBroker.lastAppId];
		double[] groupTotalServetimeNetwork = new double[SDNCustomBroker.lastAppId];
		
		for(WorkloadParser wp:workloadId.keySet()) {
			Log.printLine("testtttttttttttttttttttttttttttttt");
		...
		for(int i = 0; i< SDNCustomBroker.lastAppId; i++) {
	...
	public void submitDeployApplication(SDNDatacenter dc, String filename) {
		SDNCustomBroker.datacenters.put(dc.getName(), dc); // default DC
	...
	private void applicationSubmitCompleted(SimEvent ev) {
		for(String filename: this.workloadFileNames) {
			...
			workloadId.put(wParser, SDNCustomBroker.lastAppId);
			SDNCustomBroker.lastAppId++;
	...
	private void processApplication(int userId, String vmsFileName){
		SDNDatacenter defaultDC = SDNCustomBroker.datacenters.entrySet().iterator().next().getValue();
		VirtualWorkflowTopologyParser parser = new VirtualWorkflowTopologyParser(defaultDC.getName(), vmsFileName, userId);
		
		for(String dcName: SDNCustomBroker.datacenters.keySet()) {
			SDNDatacenter dc = SDNCustomBroker.datacenters.get(dcName);
		...
		for(FlowConfig arc:parser.getArcList()) {
			SDNDatacenter srcDc = SDNCustomBroker.vmIdToDc.get(arc.getSrcId());
			SDNDatacenter dstDc = SDNCustomBroker.vmIdToDc.get(arc.getDstId());
		...
		for(ServiceFunctionChainPolicy policy:parser.getSFCPolicyList()) {
			SDNDatacenter srcDc = SDNCustomBroker.vmIdToDc.get(policy.getSrcId());
			SDNDatacenter dstDc = SDNCustomBroker.vmIdToDc.get(policy.getDstId());
			if(srcDc.equals(dstDc)) {
				// Intra-DC traffic: create a virtual flow inside the DC
	...
	public static SDNDatacenter getDataCenterByName(String dcName) {
		return SDNCustomBroker.datacenters.get(dcName);
	}
	
	public static SDNDatacenter getDataCenterByVmID(int vmId) {
		return SDNCustomBroker.vmIdToDc.get(vmId);
	}
	...
	private void scheduleRequest(WorkloadParser workParser) {
		...
		if(parsedWorkloads.size() > 0) {
			...
			for(Workload wl: parsedWorkloads) {
				...
				wl.appId = workloadId;
				SDNDatacenter dc = SDNCustomBroker.vmIdToDc.get(wl.submitVmId);
