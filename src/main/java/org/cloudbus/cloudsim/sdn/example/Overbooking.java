...
import org.cloudbus.cloudsim.sdn.policies.vmallocation.overbooking.OverbookingVmAllocationPolicyConsolidateConnected;
import org.cloudbus.cloudsim.sdn.policies.vmallocation.overbooking.OverbookingVmAllocationPolicyPowerNet;
import org.cloudbus.cloudsim.sdn.policies.vmallocation.overbooking.OverbookingVmAllocationPolicyStaticRatio;
import org.cloudbus.cloudsim.sdn.policies.vmallocation.overbooking.VmMigrationPolicyGroupConnectedFirst;
import org.cloudbus.cloudsim.sdn.policies.vmallocation.overbooking.VmMigrationPolicyLeastCorrelated;
import org.cloudbus.cloudsim.sdn.policies.vmallocation.overbooking.VmMigrationPolicyMostFull;
import org.cloudbus.cloudsim.sdn.policies.vmallocation.overbooking.VmMigrationPolicyUnderutilizedMostFull;
...
public class Overbooking {
    ...
    enum VmAllocationPolicyEnum {
        CombLFF, CombMFF, MipLFF, MipMFF, OverLFF, OverMFF, LFF, MFF,
        OverMFF_Static,
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        String policy = "MFF"; //args[n++];

        //Configuration.OVERBOOKING_RATIO_INIT = Double.parseDouble(args[n++]);

        setExpName(policy);
        org.cloudbus.cloudsim.sdn.example.StartExperimentOverbooking.VmAllocationPolicyEnum vmAllocPolicy = org.cloudbus.cloudsim.sdn.example.StartExperimentOverbooking.VmAllocationPolicyEnum.valueOf(policy);

        if (args.length > n)
            physicalTopologyFile = "dataset-energy/energy-physical.json"; //args[n++];

        if (args.length > n)
            deploymentFile = "dataset-energy/energy-virtual.json"; //args[n++];
	...
	*/
        ...
        deploymentFile = "dataset-energy/energy-virtual.json";
        workloads = (List<String>) Arrays.asList("dataset-energy/energy-workload.csv");
        ...
        try {
            ...
            org.cloudbus.cloudsim.sdn.example.StartExperimentOverbooking.VmAllocationPolicyFactory vmAllocationFac = null;
            ...
            switch (vmAllocPolicy) {
                case CombMFF:
                case MFF:
                    vmAllocationFac = new org.cloudbus.cloudsim.sdn.example.StartExperimentOverbooking.VmAllocationPolicyFactory() {
                        public VmAllocationPolicy create(List<? extends Host> list,
                                                         HostSelectionPolicy hostSelectionPolicy,
                                                         VmMigrationPolicy vmMigrationPolicy
                        ) {
...
//			snos.setMonitorEnable(false);
            ...
            if (!org.cloudbus.cloudsim.sdn.example.Overbooking.logEnabled)
                Log.disable();
    ...
    protected static SDNDatacenter createSDNDatacenter(String name,
                                                       ...
                                                       org.cloudbus.cloudsim.sdn.example.StartExperimentOverbooking.VmAllocationPolicyFactory vmAllocationFactory,
                                                       HostSelectionPolicy hostSelectionPolicy,
                                                       VmMigrationPolicy vmMigrationPolicy) {
